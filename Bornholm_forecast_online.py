...
df= pd.read_csv ("bornholm_features_selected.csv",
                          sep=',', index_col=0)
...
for method in methods:
    ...
    if method == 'OLS_Yeo':
        y_test, y_train, sigma2s = OLS_Yeo(df_features, df_output, N_train, nu=2.9)
    if method == "RLS":
        y_test, y_train, _, sigma2s = RLS(df_features, df_output, N_train, lam_fgt=0.996)
    if method == "RLS_LG":
        y_test, y_train, _, sigma2s = RLS_LG(df_features, df_output, N_train,lam_fgt=0.996, nu=1.9)
    if method == "RLS_Yeo":
        y_test, y_train, _, sigma2s = RLS_Yeo(df_features, df_output, N_train, lam_fgt=0.996, nu=1.8)
    if method == "RML":
        y_test, y_train, _, sigma2s = RML(df_features, df_output, N_train, NUM=2000, lam_fgt=0.994)
    if method == "RML_LG":
        y_test, y_train, _, sigma2s = RML_LG(df_features, df_output, N_train, NUM=2000, lam_fgt=0.994, nu=0.2)
    if method == "RML_Yeo":
        y_test, y_train, _, sigma2s = RML_Yeo(df_features, df_output, N_train, NUM=2000, lam_fgt=0.994, nu=2.9)
...
df_origin_test.plot()
df_origin_test.to_csv("bornholm_test_pred_online.csv")
...
cols_new=['OLS', 'OLS_LG', 'OLS_Yeo', 'RLS', 'RLS_LG', 'RLS_Yeo', 'RML', 'RML_LG',
       'RML_Yeo']
nus={'OLS': 0, 'OLS_LG': 0.1, 'OLS_Yeo': 2.9, 'RLS': 0, 'RLS_LG':1.9, 'RLS_Yeo':1.8, 'RML':0, 'RML_LG':0.2,
       'RML_Yeo':2.9}
...
df_crps_time_lead.to_csv("bornholm_crps_online.csv")
