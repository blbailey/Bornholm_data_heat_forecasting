...
def addrs_gene(path_root):
    ...
    for year in years:
        for month in months:
            addrs_month=[]
            for name in names:
                file_name='Bornholm_'+name+'_'+year+month+'.csv'
                if file_name in file_list:
                    addr='Bornholm_weather/Bornholm_'+name+'_'+year+month+'.csv'
                    addrs_month.append(addr)
            if len(addrs_month)!=0:
                addrs.append(addrs_month)
    return addrs



def subtract_update_data(path_root, path_root_new, names):

    addrs = addrs_gene(path_root)
    print(addrs)
    # names=['solarflux',
    ...
    for hour_update in range(len(hours)):
        for index_var in range(6):
            ...
            dfs.to_csv(path_root_new+'/'+names[index_var] +'_'+hours[
                hour_update]+'.csv',
                       sep=',')
            # dfs.plot()
...
# ==============read data from each files in the directory of Nordhavn_weather and then put it into the Bweather as 00

# this is to read files from Nordhavn_weather and to save it to Bweather/Bornholm_weather00.csv
...
names = ['solarflux',
         ...
         'precipitation', 'windx',
         'windy']
subtract_update_data(path_root, path_root_new, names)
